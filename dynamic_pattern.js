...
class Particle{
    ...
    constructor(){
        ...
        this.size=Math.random() * 15+1; //random size between 1 and 16 px
...
function handleParticles(){
    for(let i=0; i<particlesArray.length; i++){
        ...
        for(let j=i; j<particlesArray.length; j++){
            ...
            if(dist<100){
                cxt.beginPath();
                cxt.strokeStyle=particlesArray[i].color;
                cxt.lineWidth=1;
                cxt.moveTo(particlesArray[i].x, particlesArray[i].y);
                cxt.lineTo(particlesArray[j].x, particlesArray[j].y);
                cxt.stroke();
            }
        }
